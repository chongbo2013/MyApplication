...
public class OrmLiteBaseDao<T> {
    ...
    protected OrmLiteBaseDao(Context context){
        entityClass = (Class<T>) ((ParameterizedType) getClass()
                .getGenericSuperclass()).getActualTypeArguments()[0];
        try {
            AppOrmLiteConfigInterface appOrmLiteConfigInterface = (AppOrmLiteConfigInterface) context.getApplicationContext();
            dao = BaseOrmLiteHelper.getDao(entityClass, appOrmLiteConfigInterface.getConnectionSource());
        } catch (Exception e) {
